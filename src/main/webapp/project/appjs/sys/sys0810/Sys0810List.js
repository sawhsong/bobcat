...
$(function() {
	...
	renderDataGridTable = function(result) {
		...
		if (ds.getRowCnt() > 0) {
			for (var i=0; i<ds.getRowCnt(); i++) {
				...
				uiChk.setName("chkToSave").setClassName(className+" inTblGrid").setOptions(checkString+" "+disabledStr).setValue(ds.getValue(i, "REPAYMENT_TYPE_ID")+"_"+ds.getValue(i, "REPAYMENT_TYPE_CODE"));
				gridTr.addChild(new UiGridTd().addClassName("Ct").addChild(uiChk));

				if (!commonJs.isEmpty(ds.getValue(i, "REPAYMENT_TYPE_ID"))) {
					var uiAnc = new UiAnchor();
					uiAnc.setText(ds.getValue(i, "REPAYMENT_TYPE_NAME")).setScript("getUpdate('"+ds.getValue(i, "REPAYMENT_TYPE_ID")+"', '"+ds.getValue(i, "REPAYMENT_TYPE_CODE")+"')");
					gridTr.addChild(new UiGridTd().addClassName("Lt").addChild(uiAnc));
				} else {
					gridTr.addChild(new UiGridTd().addClassName("Lt").setText(ds.getValue(i, "REPAYMENT_TYPE_NAME")));
				}
	...
	getUpdate = function(repaymentTypeId, repaymentTypeCode) {
		popup = commonJs.openPopup({
			popupId:"RepaymentTypeUpdate",
			url:"/sys/0810/getUpdate.do",
			data:{
				mode:"Update",
				repaymentTypeId:repaymentTypeId,
				repaymentTypeCode:repaymentTypeCode
			},
