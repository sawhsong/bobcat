	...
	renderDataGridTable = function(result) {
		...
		if (ds.getRowCnt() > 0) {
			for (var i=0; i<ds.getRowCnt(); i++) {
				...
				uiChk.setName("chkToSave").setClassName(className+" inTblGrid").setOptions(checkString+" "+disabledStr).setValue(ds.getValue(i, "ASSET_TYPE_ID")+"_"+ds.getValue(i, "ASSET_TYPE_CODE"));
				gridTr.addChild(new UiGridTd().addClassName("Ct").addChild(uiChk));

				if (!commonJs.isEmpty(ds.getValue(i, "ASSET_TYPE_ID"))) {
					var uiAnc = new UiAnchor();
					uiAnc.setText(ds.getValue(i, "ASSET_TYPE_NAME")).setScript("getUpdate('"+ds.getValue(i, "ASSET_TYPE_ID")+"', '"+ds.getValue(i, "ASSET_TYPE_CODE")+"')");
					gridTr.addChild(new UiGridTd().addClassName("Lt").addChild(uiAnc));
				} else {
					gridTr.addChild(new UiGridTd().addClassName("Lt").setText(ds.getValue(i, "ASSET_TYPE_NAME")));
				}
